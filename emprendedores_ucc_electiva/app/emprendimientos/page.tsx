...
const CategoryIcon = ({ nombre, size = 20, className = "" }: { nombre: string; size?: number; className?: string }) => {
  const props = {
    width: size,
    height: size,
    viewBox: "0 0 24 24",
        ...
        <section className={styles.content}>
          ...
          {!loading && !busqueda && categoriaSeleccionada === "Todas" && (
            <div className={styles.rankingSection}>
              <h2 className={styles.rankingTitle}>Los más vendidos 🔥</h2>
                ...
                return (
                  <div key={emp.id || emp._id} className={styles.card}>
                    ...
                    <div className={styles.cardButtons}>
                      <Link href={`/emprendimientos/${emp.id || emp._id}`} className={styles.cardBtn}>Ver más</Link>
                      {telefono && (
                        <a href={`https://wa.me/${formatearNumeroWhatsApp(telefono)}`} target="_blank" className={styles.whatsappBtn}>WhatsApp</a>
                      )}
                    </div>
                  </div>
                );
