...
const carritoActual: ItemCarrito[] = JSON.parse(localStorage.getItem(carritoKey) || "[]");
  ...
  const obtenerCategoria = async (categoriaId: string) => {
    try {
      const r = await fetch(`${API_BASE_URL}/api/categorias/${categoriaId}`);
      if (!r.ok) return null;
  ...
  const verificarSeguimiento = async () => {
    ...
    try {
      const empId = emprendimiento.id || emprendimiento._id;
      const r = await fetch(`${API_BASE_URL}/api/seguimientos/verificar?usuarioId=${usuarioActual.id}&emprendimientoId=${empId}`);
  ...
  const toggleSeguimiento = async () => {
    ...
    try {
      const empId = emprendimiento.id || emprendimiento._id;
      const r = await fetch(
        siguiendo ? `${API_BASE_URL}/api/seguimientos/dejar-de-seguir` : `${API_BASE_URL}/api/seguimientos/seguir`,
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        setLoading(true);
        const r = await fetch(`${API_BASE_URL}/api/emprendimientos/${id}`);
  ...
  useEffect(() => {
    if (emprendimiento && usuarioActual?.id) {
      ...
      const getCount = async () => {
        try {
          const empId = emprendimiento.id || emprendimiento._id;
          const r = await fetch(`${API_BASE_URL}/api/seguimientos/verificar?usuarioId=dummy&emprendimientoId=${empId}`);
